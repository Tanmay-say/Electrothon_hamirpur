 ...
 */
const verifyCorrectness = async (question: string, studentAnswer: string) => {
  const prompt = `
    Question: ${question}
 ...
 */
const matchDesiredAnswer = async (correctAnswer: string, studentAnswer: string) => {
  const prompt = `
 ...
 */
export const evaluateAnswer = async (question: string, desiredAnswer: string, studentAnswer: string) => {
  ...
  if (correctnessResponse.includes("Correctness: True")) {
    const matchResponse = await matchDesiredAnswer(desiredAnswer, studentAnswer);
